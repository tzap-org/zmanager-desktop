...
The current login helper uses a separate Playwright browser session; observing
the user's configured default browser remains a follow-up gate. Production and
...
runner rejects local and production environment settings. Staging still
requires an operator-approved browser profile and credentials.
