...
const windowsRootClsids = {
  archive: "d5ba8f7a-bb17-4c40-bb7b-28e971b37288",
  creation: "5af01874-4485-4fcf-b31a-37918614b6c5",
};
...
pub(crate) const ARCHIVE_ROOT_CLSID: GUID = GUID::from_u128(0x${windowsRootClsids.archive.replaceAll("-", "_")});
pub(crate) const CREATE_ROOT_CLSID: GUID = GUID::from_u128(0x${windowsRootClsids.creation.replaceAll("-", "_")});
...
${orderedContextActions("windowsExplorer", "archiveSingle").map(({ rustCase }) => `    ExplorerAction::${rustCase},`).join("\n")}
];

pub(crate) const CREATE_EXPLORER_ACTIONS: &[ExplorerAction] = &[
${orderedContextActions("windowsExplorer", "creation").map(({ rustCase }) => `    ExplorerAction::${rustCase},`).join("\n")}
];
...
const nsisBackgroundActionRows = orderedContextActions("windowsExplorer", "container")
  .map((action, index) => {
    const verb = `${String(index + 1).padStart(2, "0")}${action.nativeVerb}`;
    return `  !insertmacro ZM_WRITE_COMMAND_SUBCOMMAND_VERB "\${SUBCOMMANDS_KEY}" "${verb}" "${action.canonicalLabel}" "${action.compatibilityAliases[0]}" "%V"`;
  })
  .join("\n");
put("packaging/windows/nsis-shell-actions.generated.nsh", `; Generated by scripts/generate-native-contracts.mjs. Do not edit.
!define ZM_ARCHIVE_ROOT_CLSID "{${windowsRootClsids.archive.toUpperCase()}}"
!define ZM_CREATE_ROOT_CLSID "{${windowsRootClsids.creation.toUpperCase()}}"
...
${windowsActions.map((action) => `  !insertmacro ZM_UNREGISTER_COM_CLASS "{${action.windowsClsid.toUpperCase()}}"`).join("\n")}
!macroend

!macro ZM_REGISTER_GENERATED_BACKGROUND_SUBCOMMANDS SUBCOMMANDS_KEY
${nsisBackgroundActionRows}
!macroend
`);
