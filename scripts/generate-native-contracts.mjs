...
const nsisActionRows = (context, command) => orderedContextActions("windowsExplorer", context)
  .map((action, index) => {
    ...
    if (command) {
      return `  !insertmacro ZM_WRITE_COMMAND_SUBCOMMAND_VERB "\${SHELL_KEY}" "${verb}" "${action.canonicalLabel}" "${action.compatibilityAliases[0]}" "%V"`;
    }
    return `  !insertmacro ZM_WRITE_COM_SUBCOMMAND_VERB "\${SHELL_KEY}" "${verb}" "${action.canonicalLabel}" "\${${nsisClsidName(action)}}"`;
  })
  .join("\n");
